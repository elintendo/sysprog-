...
static void test_io(void) {
  ...
  char buffer[2048];
  unit_check(ufs_write(fd1, "123###", 3) == 3, "data (only needed) is written");
  ...
  fd1 = ufs_open("file", 0);
  unit_fail_if(fd1 == -1);
  unit_check(ufs_read(fd1, buffer, 6) == 6, "read data with zeros");
  unit_check(memcmp(buffer,
                    "1\0"
                    "2\0"
                    "3\0",
                    6) == 0,
